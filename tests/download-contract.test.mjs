...
test("current release metadata is consistent across published surfaces", async () => {
  ...
  assert.equal(current.version, "0.4.7");
  assert.equal(current.releaseDate, "2026-07-15");
  assert.equal(current.sparkleVersion, 299);
  ...
  for (const surface of [changelog, releaseNotes]) {
    assert.match(surface, /0\.4\.7/);
    assert.match(surface, /面板即计算器/);
    assert.match(surface, /单位换算/);
    assert.match(surface, /人民币大写金额/);
    assert.match(surface, /行为变更/);
  }
});
...
test("only known release assets receive binary response headers", async () => {
  ...
  const releaseFiles = (await Promise.all(["0.4.0", "0.4.1", "0.4.2", "0.4.3", "0.4.4", "0.4.5", "0.4.6", "0.4.7"].flatMap((version) => [
...
test("0.4.8 remains an unpublished skeleton with no invented release facts", () => {
  assert.equal(next.version, "0.4.8");
  ...
  for (const surface of [index, appcast, redirects, headers, changelog]) {
    assert.doesNotMatch(surface, /0\.4\.8/);
  }
});
